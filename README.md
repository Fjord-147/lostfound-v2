...
**系统只有一个初始账号**，由 `.env` 里的三个变量决定（首次 `pnpm seed` 时创建）：

| 项 | 默认值 |
|----|--------|
| 账号 | `liumin`（来自 SEED_USERNAME） |
| 密码 | `liumin123`（来自 SEED_PASSWORD） |
| 显示真名 | `刘敏`（来自 SEED_NAME，用于登记/认领留痕） |

- **没有** admin、guide1 之类的账号；忘记密码/换密码 → 改 `.env` 的 SEED_* 后删库重 seed，或直接改数据库 users 表（bcrypt）
- 加新护士账号 → 往 users 表插入（后续可在管理端做账号管理页）
- SSO 接入后此账号体系停用，改用员工门户统一登录（见下文 SSO 章节）
